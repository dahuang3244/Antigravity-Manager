...
*   **版本演进 (Changelog)**:
    *   **v3.3.49 (2026-01-22)**:
        -   **[核心修复] 流式响应 Thinking Block 顺序修正 (Fix PR #949)**:
            -   **问题背景**: 解决了一些特定的流式输出中，思考块（Thinking Block）与普通文本块顺序错乱导致的 400 校验错误。
            -   **修复内容**: 优化了流式响应收集器的缓冲区逻辑，确保所有思考内容在发送前完成正确的排序与封装。
        -   **[性能优化] 设置项即时生效与同步策略更新**:
            -   **即时生效**: 修复了语言和主题切换需要手动点击保存的问题。现在修改设置会立即应用到整个 UI 并同步持久化到全局配置。
            -   **状态同步**: 解决了 `App.tsx` 与 `Settings.tsx` 之间的状态冲突，确保配置加载过程中的 UI 一致性。
    *   **v3.3.48 (2026-01-21)**:
